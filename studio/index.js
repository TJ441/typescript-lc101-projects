...
for (var i = 0; i < astronauts.length; i++) {
    var status_1 = '';
    if (falcon9.addAstronaut(astronauts[i])) {
        status_1 = "On board";
    }
    else {
        status_1 = "Not on board";
    }
    console.log("".concat(astronauts[i].name, ": ").concat(status_1));
}
var cargo = [
...
for (var i = 0; i < cargo.length; i++) {
    var loaded = '';
    if (falcon9.addCargo(cargo[i])) {
        loaded = "Loaded";
    }
    else {
        loaded = "Not loaded";
    }
    console.log("".concat(cargo[i].material, ": ").concat(loaded));
}
